...
//!   doctests;
//! - the coverage steps in `ci.yml` and `coverage-main.yml` pass every declared feature, so the
//!   coverage run selects the same tests as `make test`'s `--all-features`.
//!
...
const COVERAGE_FEATURES: &str = "test-util";

/// The workflows whose coverage step must pass the features.
const COVERAGE_WORKFLOWS: [&str; 2] = ["ci.yml", "coverage-main.yml"];

/// Returns every feature the crate declares, sorted, less `default`.
fn declared_features() -> std::io::Result<Vec<String>> {
    let mut found: Vec<String> = manifest("Cargo.toml")?
        .features()
        .into_iter()
...
fn coverage_enables_every_declared_feature() {
    ...
    passed.sort();
    assert_eq!(
        declared_features().expect("failed to read Cargo.toml"),
        passed,
